...
unique(turnover_nuts_size$NUTS)

tab_ex2 <- turnover_nuts_size %>% 
...
masq_gauss <- tab_ex2_pr_gauss %>% 
  GaussSuppressionFromData(
    ...
    protectZeros = FALSE, 
    secondaryZeros = TRUE,
    hierarchies = list(NUTS=hier_nuts, SIZE = hier_SIZE)
  )
...
masq_ta <- tab_rtauargus(
  ...
  files_name = "ex2",
  explanatory_vars = c("NUTS","SIZE"), #equivalent de DimVar
  value = "N_OBS",
  freq = "N_OBS", #freqVAR
  secret_var = "is_secret_prim",
  hrc = c(NUTS = hrc_file_nuts),
  totcode = c(NUTS="Total",SIZE="Total"),
...
lignes_en_plus <- anti_join(masq_gauss, masq_ta, by = c("NUTS", "SIZE"))
lignes_en_plus
...
masq_gauss <- anti_join(masq_gauss, lignes_en_plus, by = c("NUTS", "SIZE") )
...
masq_gauss %>% 
  ...
  select(NUTS, SIZE, N_OBS, Status_Gauss) %>% 
  full_join(
    masq_ta %>% select(NUTS, SIZE, N_OBS, Status),
    by = c("NUTS", "SIZE", "N_OBS")
  ) %>% 
  mutate(
...
# }
# 
# filtered_data <- filter_data_by_division(masq_gauss, hier_nuts, "01")
# 
# print(filtered_data)
# 
# df<-hier_nuts
...
merged_data <- inner_join(filtered_data2_gauss, filtered_data2_ta, by = c("NUTS","SIZE","N_OBS"))
merged_data <- merged_data[,c("NUTS","SIZE","N_OBS","primary","suppressed","is_secret_prim","Status")]

print(merged_data)
merged_data_visuel <- merged_data[,c("NUTS","SIZE","N_OBS")] %>% 
  pivot_wider(names_from = "NUTS", values_from = 'N_OBS')
...
merged_data <- inner_join(filtered_data2_gauss, filtered_data2_ta, by = c("NUTS","SIZE","N_OBS"))
merged_data <- merged_data[,c("NUTS","SIZE","N_OBS","Status_Gauss","Status")]
...
lignes_en_plus <- anti_join(filtered_data2_gauss, filtered_data2_ta, by = c("NUTS", "SIZE"))
lignes_en_plus
...
#   filtered_data2_ta %>% filter(NUTS != "BE"),
#   dir_name = "tauargus_files/ex2",
#   files_name = "ex2",
#   explanatory_vars = c("NUTS","SIZE"), #equivalent de DimVar
#   value = "N_OBS",
